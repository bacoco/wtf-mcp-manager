...
class WTFMCPManagerServer {
  ...
  async queryRouterHTTP(query, limit = 10) {
    ...
    try {
      const results = await this.routerClient.query({ query, topK: limit });
  ...
  async searchMCPs(query, options = {}) {
    ...
    const httpResults = await this.queryRouterHTTP(sanitizedQuery, limit);
    ...
    try {
      return await this.routerRetriever.query(sanitizedQuery, {
        limit,
        registryText: this.registryText,
        availableMCPs: this.availableMCPs
      });
    } catch (error) {
      console.error('Local MCP search failed, falling back to keyword search:', error.message || error);
      return this.keywordFallbackSearch(sanitizedQuery, limit);
    }
  }
  ...
  async searchMCPsLocal(query, limit = 10) {
    ...
    if (this.vectorRouter?.query) {
      try {
        if (this.vectorRouter?.isAvailable?.() === false) {
          throw new VectorStoreUnavailableError('Vector router is unavailable');
        }

        const vectorResults = await this.vectorRouter.query(sanitizedQuery, limit);
        if (Array.isArray(vectorResults) && vectorResults.length > 0) {
          return vectorResults;
        }
      } catch (error) {
        if (error instanceof VectorStoreUnavailableError || error?.name === 'VectorStoreUnavailableError') {
