...
export class MetaAPIAggregator {
  ...
  async searchAPIsGuru(query) {
    try {
      ...
      if (!response.ok) {
        console.error('APIs.guru fetch failed:', response.status);
        return null;
      }
      ...
      for (const [key, api] of Object.entries(data)) {
        if (api.preferred) {
          ...
          // Maritime-specific scoring
          if (queryWords.some(w => ['vessel', 'ship', 'maritime', 'marine', 'ais', 'tracking'].includes(w))) {
      ...
      // Sort by relevance
      apis.sort((a, b) => b.relevanceScore - a.relevanceScore);
