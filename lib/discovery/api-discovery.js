...
export class APIDiscoveryService {
  ...
  async discoverAllAPIs(query, options = {}) {
    ...
    // Enrich top results with full specifications
    if (metaResults.apis && metaResults.apis.length > 0) {
      const enrichmentPromises = metaResults.apis
        .slice(0, 5) // Enrich top 5 APIs with full specs
        .map(api => this.getAPISpecification(api).catch(err => api));

      const enrichedAPIs = await Promise.allSettled(enrichmentPromises);

      // Replace top APIs with enriched versions
      enrichedAPIs.forEach((result, index) => {
        if (result.status === 'fulfilled' && result.value.hasSpec) {
          metaResults.apis[index] = result.value;
        }
      });
    }
  ...
  async getAPISpecification(api) {
    // If API has an OpenAPI spec URL (from APIs.guru or other sources)
    if (api.swaggerUrl || api.openApiSpec) {
      const specUrl = api.swaggerUrl || api.openApiSpec;
      try {
        ...
        console.error('Failed to fetch OpenAPI spec from', specUrl, ':', error.message);
      }
    }
