...
export function parseNodeVersion(versionString = process.versions?.node || process.version) {
  if (!versionString || typeof versionString !== 'string') {
    return null;
  }

  const match = /^v?(\d+)(?:\.(\d+))?/.exec(versionString.trim());
  if (!match) {
    return null;
  }
...
`;
  ...
  async diagnose() {
    ...
    const versionString = process.versions?.node || process.version;
    const nodeVersion = parseNodeVersion(versionString);
    if (nodeVersion && nodeVersion.major < 18) {
      issues.push({
        type: 'warning',
        message: `Node.js ${versionString} is outdated`,
